...
public class Enemy : MonoBehaviour, IBattle, IHealth
{
    ...
    [System.Serializable]
    public struct ItemDropInfo          // 드랍아이템 정보
    {
            ...
            if (state != value)
            {
                ...
                switch (state)  // 새로운 상태(새로운 상태로 들어가면서 해야 할 일 처리)
                {
                    ...
                    case EnemyState.Dead:
                        ...
                        StartCoroutine(DeadRepresent());    // 시간이 지나면 서서히 가라앉는다.
    ...
    {
        // 도착확인
        // agent.pathPending : 경로계산이 진행중인지 확인. true면 아직 경로 계산 중
    ...
    bool IsSightBlock(Vector3 toTargetDir)
    {
        bool result = true;
    ...
    void MakeDropItem()
    {
        ...
        for(int i=0; i<dropItems.Length; i++)
        {
            ...
            // checkPercentage와 percentage를 비교 (랜덤 숫자가 누적된 확율보다 낮은지 확인, 낮으면 해당 아이템 생성)
