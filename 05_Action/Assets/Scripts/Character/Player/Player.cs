...
public class Player : MonoBehaviour, IBattle, IHealth
{
    ...
    private void Awake()
    {
        weapon_r = GetComponentInChildren<WeaponPosition>().transform;  // 무기가 붙는 위치를 컴포넌트 찾기

        // 장비교체가 일어나면 새로 설정해야 한다.
        weaponPS = weapon_r.GetComponentInChildren<ParticleSystem>();   // 무기가 붙어있는 파티클 시스템 가져오기
    ...
    /// 무기의 이팩트를 키고 끄는 함수
    /// </summary>
    /// <param name="on">true면 무기 이팩트를 켜고 false면 무기 이팩트를 끈다.</param>
